...
int RandomNumber(unsigned int num) {
  unsigned int a = t1;
...
void CreateMaze() {
  ...
  unsigned int a = RandomNumber(num);
  a += 0x14;
  SizeMaze = a;
  unsigned int b = 0;
  ...
  num = SizeMaze;
  start.x = RandomNumber(num);
  start.y = RandomNumber(num);

  do {
    num = SizeMaze;
    finish.x = RandomNumber(num);
  } while (start.x == finish.x);

  do {
    num = SizeMaze;
    finish.y = RandomNumber(num);
